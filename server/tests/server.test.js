...
const {todos, populateTodos, users, populatUsers} = require('./seed/seed');
...
describe('POST /todos', () => {
  ...
  it('should not create to do with invalid', (done) => {
    request(app)
      .post('/todos')
      .set('x-auth', users[0].tokens[0].token)
      .send({})
...
describe('GET /todos', () => {
  ...
  describe('GET /todos/:id', () => {
    ...
    it('should not todo by id created by ID', (done) => {
      request(app)
...
describe('PATCH/todo/:id', () => {
  it('should update a todo', (done) => {
    var hexId = todos[0]._id.toHexString();
    var tmpText = "updated for test 1";
    request(app)
      .patch(`/todos/${hexId}`)
      .set('x-auth', users[0].tokens[0].token)
      .send({
      ...
      .expect(200)
      .expect((res) => {
        expect(res.body.todo.text).toBe(tmpText);
  ...
  it('should not update a todo not owned by user', (done) => {
    var hexId = todos[0]._id.toHexString();
    var tmpText = "updated for test 1";
    request(app)
      .patch(`/todos/${hexId}`)
